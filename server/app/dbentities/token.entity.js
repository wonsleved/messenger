...
class TokenEntity {
  static async insertRefreshToken(userId, refreshToken) {
    const queryResult =
      await db.query('INSERT INTO "token" (user_id, refresh_token) ' +
  ...
  static async selectByRefreshToken(refreshToken) {
    const queryResult =
      await db.query('SELECT * FROM "token" WHERE refresh_token=$1;',
        [refreshToken]);
  ...
  static async selectByUserId(userId) {
    const queryResult =
      await db.query('SELECT * FROM "token" WHERE user_id=$1;',
        [userId]);
